...
#include "enlightning_hydro_K.H"
#include "enlightning_K.H"

using namespace amrex;

Real
enlightning::advance (Real time, Real dt, int iteration, int ncycle)
{
    BL_PROFILE("enlightning::advance()");
...
void
enlightning::compute_dSdt (const MultiFab& S, MultiFab& dSdt, Real dt,
                   FluxRegister* fr_as_crse, FluxRegister* fr_as_fine)
{
    BL_PROFILE("enlightning::compute_dSdt()");
    ...
    for (MFIter mfi(S); mfi.isValid(); ++mfi)
    {
        ...
        {
            enlightning_ctoprim(i, j, k, sfab, q);
        });
        ...
        [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
        {
            enlightning_slope_x(i, j, k, slope, q);
        });
        const Box& xflxbx = amrex::surroundingNodes(bx,cdir);
        amrex::ParallelFor(xflxbx,
        [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
        {
            enlightning_riemann_x(i, j, k, fxfab, slope, q);
        ...
        [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
        {
            enlightning_slope_y(i, j, k, slope, q);
        });
        const Box& yflxbx = amrex::surroundingNodes(bx,cdir);
        amrex::ParallelFor(yflxbx,
        [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
        {
            enlightning_riemann_y(i, j, k, fyfab, slope, q);
        ...
        [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
        {
            enlightning_slope_z(i, j, k, slope, q);
        });
        const Box& zflxbx = amrex::surroundingNodes(bx,cdir);
        amrex::ParallelFor(zflxbx,
        [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
        {
            enlightning_riemann_z(i, j, k, fzfab, slope, q);
        ...
        {
            enlightning_flux_to_dudt(i, j, k, n, dsdtfab, AMREX_D_DECL(fxfab,fyfab,fzfab), dxinv);
        });
